/**
 * Pure-fn unit tests for the x402 header builder.
 * No Solana RPC needed.
 */
...
describe("x402 header builder", () => {
  it("Allow → 200 + decision header", () => {
    ...
    expect(headers[X_PAYMENT_REQUIRED]).to.be.undefined;
  });
  ...
  it("Deny → 402 + reason headers", () => {
    const { httpStatus, headers } = buildHeadersForDecision({
      kind:       "Deny",
      reasonCode: 6,
      reasonName: "CounterpartyTierBelowMin",
    });
...
describe("denyReasonName", () => {
  it("maps known codes to playbook names", () => {
    ...
    expect(denyReasonName(2)).to.equal("SpendingPerTxExceeded");
