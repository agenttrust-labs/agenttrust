#!/usr/bin/env node
/**
 * AgentTrust 0.4.0 Gate E2E driver — run-3
 * (against @agenttrust-sdk/mcp@0.4.3 with refreshed bundled IDL
 *  after the on-chain init_if_needed upgrade on TrustGateAuthority.)
 *
 * Spawns `npx -y @agenttrust-sdk/mcp@0.4.3` in stdio mode with a fresh
...
console.error('[gate] spawning npx -y @agenttrust-sdk/mcp@0.4.3 ...');
...
const child = spawn('npx', ['-y', '@agenttrust-sdk/mcp@0.4.3'], {
...
(async () => {
  ...
  const initPolicyArgs = {
    // 0.4.3: omit agent_asset; init_policy generates a fresh ephemeral
