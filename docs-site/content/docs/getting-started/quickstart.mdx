...
```bash
pay --sandbox curl https://agenttrust-demo.fly.dev/protected
```

For a deterministic Allow path, use the tier-3 demo counterparty.

```bash
PAYER=$(curl -s https://agenttrust-demo.fly.dev/health | jq -r '.counterparties[2].agent')
pay --sandbox curl -H "X-Demo-Payer-Agent: $PAYER" https://agenttrust-demo.fly.dev/protected
```

For a deterministic Deny path, use tier 0.

```bash
PAYER=$(curl -s https://agenttrust-demo.fly.dev/health | jq -r '.counterparties[0].agent')
pay --sandbox curl -H "X-Demo-Payer-Agent: $PAYER" https://agenttrust-demo.fly.dev/protected
```
