...
`agenttrust_init_policy` is the single bootstrap call. On a fresh wallet it self-heals two missing accounts in the same atomic transaction before initialising the policy:

1. The Quantu `agent_account` plus `atom_stats` PDA, chained through TrustGate's `register_agent_via_cpi`. Your wallet ends up atom-functional and registered against Quantu's `agent_registry_8004` without ever learning Quantu's instruction names.
2. The AgentTrust `PolicyAuthority` PDA, via `init_authority` (single member = your signer, threshold = 1).

Then the policy itself plus a `VelocityLedger`. Three on-chain account creations and the new policy, one signed transaction. You never see Anchor error 3012 and you never run a bootstrap script. Spending caps also default sanely: when at least one cap is set, unspecified peer caps default to the max of the specified caps rather than zero, so v1 policies (immutable post-init) cannot accidentally hard-deny every payment.
...
Open the `explorerTxUrl` to see all three account creations and the policy init in one tx. Second-run of the same tool on the same wallet returns `healedSteps: []` — the self-heal is idempotent because Quantu's `register_with_options` and the AgentTrust `init_authority` both fail at their `init` constraints once the PDAs exist, and the tool's pre-flight check skips the prepend.
