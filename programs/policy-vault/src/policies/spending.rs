...
pub fn evaluate(state: SpendingState, amount: u64, unix_ts: i64) -> SpendingOutcome {
    ...
    let week_used_effective = if state.week_anchor == week_anchor_now {
        state.week_used
    } else {
        0
    };
...
mod tests {
    ...
    fn deny_per_tx_exceeded() {
        ...
        assert_eq!(
            out,
            SpendingOutcome::Deny(DenyReason::SpendingPerTxExceeded)
        );
    }
    ...
    fn deny_daily_exceeded_with_existing_usage() {
        ...
        assert_eq!(
            out,
            SpendingOutcome::Deny(DenyReason::SpendingDailyExceeded)
        );
    }
    ...
    fn deny_weekly_exceeded_with_existing_usage() {
        ...
        assert_eq!(
            out,
            SpendingOutcome::Deny(DenyReason::SpendingWeeklyExceeded)
        );
    }
