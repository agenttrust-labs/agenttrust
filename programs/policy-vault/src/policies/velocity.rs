...
pub fn evaluate(
    ...
    let active_in_window = if window_expired {
        0
    } else {
    ...
    VelocityOutcome::Allow(VelocityDeltas {
        ...
        new_last_commit_slot: now_slot,
        new_window_start_slot: if window_expired {
            now_slot
        } else {
            ledger.window_start_slot
        },
    })
}
...
mod tests {
    ...
    fn cumulative_one_over_max_denies() {
        ...
        assert_eq!(
            out,
            VelocityOutcome::Deny(DenyReason::VelocityWindowExceeded)
        );
    }
