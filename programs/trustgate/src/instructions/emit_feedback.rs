...
pub fn handler<'info>(
    ...
    require!(endpoint.len()     <= MAX_ENDPOINT_LEN, TrustGateError::EndpointTooLong);
    require!(feedback_uri.len() <= MAX_URI_LEN,   TrustGateError::UriTooLong);
    ...
    let cpi_accounts = unpack_cpi_accounts(ctx.remaining_accounts, &ctx.accounts.authority.to_account_info())?;
